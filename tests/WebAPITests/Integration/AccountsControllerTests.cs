...
namespace WebAPITests.Integration
{
    public class AccountsControllerTests : IClassFixture<AccountContextBuilder>, IDisposable
    {
        ...
        {
            var user = await _sharedContext.GetBasicConfirmUser();
        ...
        private void CheckJwt(string jwt)
        {
            ...
            tokenValidatorParameters.IssuerSigningKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(_sharedContext.JwtSettings.IssuerSigningKey));
            tokenValidatorParameters.ValidIssuer = _sharedContext.JwtSettings.Issuer;
            tokenValidatorParameters.ValidAudience = _sharedContext.JwtSettings.Audience;
        ...
        [Fact]
        async Task LoginAsync_forUserThatAlreadyHasAToken_RemovesOldToken()
        {
            var user = await _sharedContext.GetBasicConfirmUser();
            ...
            await _client.SendAsync(requestLogin);

            _sharedContext.IdentityDbContext.RefreshTokens.Count().Should().Be(1);
